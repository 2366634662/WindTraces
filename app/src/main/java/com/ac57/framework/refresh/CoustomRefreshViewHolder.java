 ...
 */
package com.ac57.framework.refresh;
...
public class CoustomRefreshViewHolder extends RefreshViewHolder {
     ...
     */
    public CoustomRefreshViewHolder(Context context, boolean isLoadingMoreEnabled) {
    ...
    public View getRefreshHeaderView() {
        if (mRefreshHeaderView == null) {
            ...
            refreshView = (RefreshView) mRefreshHeaderView.findViewById(R.id.meiTuanView);
            if (mPullDownImageResId != -1) {
                refreshView.setPullDownImageResource(mPullDownImageResId);
            } else {
                refreshView.setPullDownImageResource(R.drawable.dropdown_animation1);
//                throw new RuntimeException("请调用" + BGAMeiTuanRefreshViewHolder.class.getSimpleName() + "的setPullDownImageResource方法设置下拉过程中的图片资源");
            ...
            if (mChangeToReleaseRefreshAnimResId != -1) {
                refreshView.setChangeToReleaseRefreshAnimResId(mChangeToReleaseRefreshAnimResId);
            } else {
                refreshView.setChangeToReleaseRefreshAnimResId(R.drawable.pull_to_refresh_header);
//                throw new RuntimeException("请调用" + BGAMeiTuanRefreshViewHolder.class.getSimpleName() + "的setChangeToReleaseRefreshAnimResId方法设置进入释放刷新状态时的动画资源");
            ...
            if (mRefreshingAnimResId != -1) {
                refreshView.setRefreshingAnimResId(mRefreshingAnimResId);
            } else {
                refreshView.setRefreshingAnimResId(R.drawable.pull_to_refresh_header);
//                throw new RuntimeException("请调用" + BGAMeiTuanRefreshViewHolder.class.getSimpleName() + "的setRefreshingAnimResId方法设置正在刷新时的动画资源");
    ...
    public void handleScale(float scale, int moveYDistance) {
        if (scale <= 1.0f) {
            refreshView.handleScale(scale);
        }
    }

    @Override
    public void changeToIdle() {
        refreshView.changeToIdle();
    }

    @Override
    public void changeToPullDown() {
        refreshView.changeToPullDown();
    }

    @Override
    public void changeToReleaseRefresh() {
        refreshView.changeToReleaseRefresh();
    }

    @Override
    public void changeToRefreshing() {
        refreshView.changeToRefreshing();
    }

    @Override
    public void onEndRefreshing() {
        refreshView.onEndRefreshing();
    }
}