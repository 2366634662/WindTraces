...
public class DiskLruCacheHelper {
    ...
    private static volatile DiskLruCacheHelper mDiskLruCacheHelper;

    public static DiskLruCacheHelper getIns(Context context) {
...
//    }
    ...
    private DiskLruCache generateCache(Context context, File dir, int maxCount) throws IOException {
        ...
        int appVersion = context == null ? DEFAULT_APP_VERSION : DiskCacheUtils.getAppVersion(context);
    ...
    private DiskLruCache generateCache(Context context, String dirName, int maxCount) throws IOException {
        DiskLruCache diskLruCache = DiskLruCache.open(
                ...
                DiskCacheUtils.getAppVersion(context),
                DEFAULT_VALUE_COUNT,
                maxCount);
        return diskLruCache;
    }
    ...
    public String getAsString(String key) {
        ...
        String str = null;
        try {
            str = DiskCahceUtil.readFully(new InputStreamReader(inputStream, DiskCahceUtil.UTF_8));
        } catch (IOException e) {
            e.printStackTrace();
            try {
                inputStream.close();
    ...
    public void put(String key, Bitmap bitmap) {
        put(key, DiskCacheUtils.bitmap2Bytes(bitmap));
    }

    public Bitmap getAsBitmap(String key) {
        byte[] bytes = getAsBytes(key);
        if (bytes == null) return null;
        return DiskCacheUtils.bytes2Bitmap(bytes);
    }
    ...
    public void put(String key, Drawable value) {
        put(key, DiskCacheUtils.drawable2Bitmap(value));
    }
    ...
    public Drawable getAsDrawable(String key) {
        ...
        return DiskCacheUtils.bitmap2Drawable(DiskCacheUtils.bytes2Bitmap(bytes));
    }
    ...
    public boolean remove(String key) {
        try {
            key = DiskCacheUtils.hashKeyForDisk(key);
    ...
    public InputStream get(String key) {
        try {
            DiskLruCache.Snapshot snapshot = mDiskLruCache.get(DiskCacheUtils.hashKeyForDisk(key));
